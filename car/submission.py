...
import util, math, random, collections, numpy
...
class ExactInference(object):
    ...
    def elapseTime(self):
        ...
        for i in range(self.belief.getNumRows()):
            for j in range(self.belief.getNumCols()):
                old[(i,j)] = self.belief.getProb(i,j)
                self.belief.setProb(i,j,0)
        for keys, trans in self.transProb.items():
            a = self.belief.getProb(keys[1][0], keys[1][1])
            self.belief.setProb(keys[1][0], keys[1][1], a+old[keys[0]]*trans)
#        A = self.transProb
#        for i in range(self.belief.getNumRows()):
#            for j in range(self.belief.getNumCols()):
#                update = 0
#                for k1 in range(self.belief.getNumRows()):
#                    for k2 in range(self.belief.getNumCols()):
#                        if ((k1,k2),(i,j)) in A and (k1,k2)!=(i,j):
#                            update += float(A[((k1,k2),(i,j))])*self.belief.getProb(k1,k2)
#                self.belief.setProb(i, j, update)
        self.belief.normalize()      
        # END_YOUR_CODE
